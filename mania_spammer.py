...
# Hold the whole chord for 14-42 ms, then wait between chord presses.
BASE_TAP_MIN_DOWN = 0.014
BASE_TAP_MAX_DOWN = 0.042
# Slow key frequency by ~50% (double-ish the gap range).
...
CONFIG_PATH = Path(__file__).with_name("mania_spammer_config.json")
...
# Keys to spam simultaneously.
CHORD_KEYS = tuple(KeyCode.from_char(ch) for ch in ("d", "f", "g", "h", "j"))
...
def tap_chord(keys: tuple[KeyCode, ...]) -> None:
    # Press everything down first, hold, then release everything.
    for k in keys:
        keyboard.press(k)
    try:
        with speed_lock:
...
def main() -> None:
    ...
    # Keep the original program's "burst loop" shape, but spam the full chord.
    burst_seconds = 35
        ...
        try:
            while not stop_event.is_set():
                if pause_event.is_set():
                    time.sleep(0.1)
                    continue
                rapid_chord_taps(burst_seconds)
